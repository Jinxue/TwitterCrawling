 ...
 * @author Jinxue Zhang
 */
public class FindTargetUsersMulti {
	...
	FindTargetUsersMulti(String propFile, String access, 
			String userF, String loc){
		...
		this.location = loc.toLowerCase();
	}

	private void buildOutFile(String userId) {
		// TODO Auto-generated method stub
		try {
			FileWriter outFile = new FileWriter("targetUsers-" + 
					userId + ".txt", true);
		...
		for (Long usr : users) {
			...
			try {
				...
				if(tuser.getLocation() != null){
					...
					if (loc.toLowerCase().contains(this.location)){
						System.out.println(userIndex + "/" + userTotal + "," + foundUsers + "," + username + ", " + friends + "," + followers);
	...
	public static void main(String[] args) {
        ...
        if (args.length < 4) {
            System.out.println("Usage: java -jar crawling.GetFriendsMulti.jar properFile accessFile UserFile Location");
            System.exit(-1);
        }
		...
		FindTargetUsersMulti userTL = new FindTargetUsersMulti(args[0], args[1], args[2], args[3]);
		userTL.doGet();
